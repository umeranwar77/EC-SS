...
def bike_detection(frame, license_plate,status,detection_type, cropedframe=None):
    try:
        model_path = "model/bike_model.pt"
...
def vehicle_detection(frame, license_plate, cropedframe=None):
    try:
            ...
            for box in boxes:
                ...
                if cls in [3, 4]:  
                    if x1 < x2 and y1 < y2:  
                        has_seatbelt = (cls == 4)
                        if license_plate:
                            license_plate.has_seatbelt = True if has_seatbelt else False
                            license_plate.confidence = f"{conf * 100:.2f}%"
                            print(license_plate)
                        detection_crop = frame[y1:y2, x1:x2]
                        if detection_crop.size > 0: 
    ...
    return frame, detections
# camera_url = '/home/umar/Desktop/Project/ECnSS/ECSS/22.mp4'
...
def process_camera_stream(camera,stop_signal):
    try:
        ...
        while not stop_signal.is_set() and not global_stop_signal.is_set():
                ...
                vehicle_model_path = "model/yolov10x.pt"
                license_plate_model_path = "model/best.pt"    
                ...
                while not stop_signal.is_set() and not global_stop_signal.is_set():
                    ...
                    if not ret or frame is None or frame.size == 0:
                        print("Video completed or invalid frame received. Stopping detection.")
                        stop_signal.set() 
                        break           
                        ...
                        for vehicle in vehicle_results[0].boxes:
                            if len(vehicle.xyxy) > 0:
                                ...
                                if class_index in CLASS_MAP:
                                    ...
                                    print(f"Processing detection: Class {class_index}, Class name: {class_name}, Coords: ({x1},{y1},{x2},{y2})")
                                            ...
                                            if class_index in CLASS_MAP:
                                                ...
                                                for plate in plate_results[0].boxes:
                                                    ...
                                                    if plate_text:
                                                        ...
                                                        elif detection_type == '0':  # Check-out logic
                                                            if last_record and last_record.detection_type == '1' and last_record.check_out_time is None:
                                                                ...
                                                                license_plate = last_record  # Set license_plate to the updated record
                                                            else:
                                                                status = "Ignored ❌ (No active check-in found)"
                                                                license_plate = last_record  # Set license_plate to last record even if invalid

                                                        # Always send WebSocket message regardless of detection type
                                                        websocket_data = {
                                                            "type": "send_detection",
                                                            "data": {
                                                                "vehicle_id": obj_id,
                                                        ...
                                                        # Process images only if we have a valid license_plate record
                                                        if license_plate:
                                                            ...
                                                            # Trigger appropriate detection based on vehicle type
                                                            if class_name.lower() in ["car", "truck", "bus"]:
                                                                vehicle_detection(vehicle_crop, license_plate, annotated_frame)
